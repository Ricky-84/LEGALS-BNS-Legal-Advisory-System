// ============================================
// Neo4j Incremental Import - 22 New BNS Sections
// ============================================
...
MERGE (s:Section {section_id: row.s_section_id})
...
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
MATCH (s:Section {section_id: row.s_section_id})
...
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
MATCH (s:Section {section_id: row.s_section_id})
MERGE (p:Punishment {section_id: row.s_section_id})
SET p.description = row.punishment,
    p.section_id = row.s_section_id
MERGE (s)-[:PRESCRIBES]->(p);

// STEP 5: Create MensRea elements for new sections
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
WHERE row.mens_rea IS NOT NULL AND trim(row.mens_rea) <> ''
MATCH (s:Section {section_id: row.s_section_id})
MATCH (o:Offence {offence_id: row.offence_type})
WITH s, o, row, split(row.mens_rea, '|') AS mens_rea_list
UNWIND mens_rea_list AS mens_rea_item
WITH s, o, row, trim(mens_rea_item) AS mens_rea_name
WHERE mens_rea_name <> ''
...
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
WHERE row.actus_reus IS NOT NULL AND trim(row.actus_reus) <> ''
MATCH (s:Section {section_id: row.s_section_id})
MATCH (o:Offence {offence_id: row.offence_type})
WITH s, o, row, split(row.actus_reus, '|') AS actus_reus_list
UNWIND actus_reus_list AS actus_reus_item
WITH s, o, row, trim(actus_reus_item) AS actus_reus_name
...
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
WHERE row.circumstances IS NOT NULL AND trim(row.circumstances) <> ''
MATCH (s:Section {section_id: row.s_section_id})
MATCH (o:Offence {offence_id: row.offence_type})
WITH s, o, row, split(row.circumstances, '|') AS circumstances_list
UNWIND circumstances_list AS circumstance_item
WITH s, o, row, trim(circumstance_item) AS circumstance_name
...
LOAD CSV WITH HEADERS FROM 'file:///bns_22_new_sections.csv' AS row
WHERE row.basic_keywords IS NOT NULL AND trim(row.basic_keywords) <> ''
MATCH (o:Offence {offence_id: row.offence_type})
