#!/usr/bin/env python3
"""
LEGALS Ollama + Phi-3 Integration Test
...
def test_entity_extraction_service():
    ...
    try:
        ...
        # Show property value analysis if available
        if "property_value_analysis" in entities:
            analysis = entities["property_value_analysis"]
            print(f"  Property Value Analysis: Total Rs.{analysis.get('total_estimated_value', 0)}")
...
def test_complete_pipeline():
    ...
    try:
        ...
        # Test the complete pipeline (would normally include Ollama extraction)
        # For now, we'll test just the legal reasoning part
        simulated_entities = {
            "persons": ["someone"],
        ...
        print("Testing legal reasoning with simulated entities...")
        legal_analysis = legal_service._legal_reasoning_step(simulated_entities)

        print("SUCCESS: Complete pipeline test!")
        print(f"Confidence: {legal_analysis['confidence_score']}")
        print(f"Laws found: {len(legal_analysis['applicable_laws'])}")
        print(f"Property analysis: {legal_analysis.get('property_value_analysis', False)}")
